...
UI-only Control Panel scaffolding has started in the frontend repo. Backend implementation is now in place, so the next step is live Control Plane data integration and guarded action wiring in `cloudpulse-ui`.
...
For frontend live/backend integration:
...
- Add `/api/control-plane/*` client helpers against the implemented backend routes.
...
- Backend status as of 2026-04-30: `user-service` role support is verified, `control-plane-service` is implemented/deployed, live `/api/control-plane/*` read APIs are active, and guarded scale `0/1` with typed confirmation + audit is implemented.
